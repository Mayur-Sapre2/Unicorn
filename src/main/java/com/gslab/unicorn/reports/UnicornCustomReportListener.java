 ...
 */
public class UnicornCustomReportListener {
    ...
    private String reportTitle = "Unicorn Customized Report";
    private String reportFileName = "Unicorn-Emailable-report.html";
     ...
     * @param reportsLocation location where to generate reports.
     */
    public void generateEmailableReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
                                        String reportsLocation) {
        try {
            writer = createWriter(Config.getReportsLocation());
        } catch (IOException e) {
        ...
        writer.println("<div class=\"container\" style=\"background-color:#DCDCDC\" style=\"padding-left: 30px;\">");
        writer.println("<div style=\"background-color:#404040\" align=\"center\">");
        writer.println("<h2 style=\"color:#FFFFFF;\"> <b>Summary</b></h2>");
        writer.println("</div>");
        generateEnvironmentReport();
        generateOverallExecutionReport(suites);
        writer.println("</div> <br>");
        //generateSuiteSummaryReport(suites);
        generateTestsSummaryReport(suites);
     ...
     * the method details
     */
    private void generateTestsSummaryReport(List<ISuite> suites) {
        m_methodIndex = 0;
        writer.println("<div  style=\"background-color:#DCDCDC\">");
        writer.println("<div style=\"background-color:#404040\" align=\"center\">");
        writer.println("<h2 style=\"color:#FFFFFF;\"> <b>Tests Summary</b></h2>");
        writer.println("</div>");
        //startResultSummaryTable("Tests Overview");
        int testIndex = 1;
           ...
           /* if (suites.size() >= 1) {
            ...
            for (ISuiteResult r2 : r.values()) {
                ...
                if (total_tests_failed > 0) {
                    writer.println("<div align='left' style=\"padding-left: 30px;\">");
                    writer.println("<h3> <b>Test Cases Failed</b></h3>");
                    startResultSummaryTable("Tests Overview failed");
                ...
                if (total_tests_skipped > 0) {
                    writer.println("<div align='left' style=\"padding-left: 30px;\">");
                    writer.println("<h3> <b>Test Cases Skipped</b></h3>");
                    startResultSummaryTable("Tests Overview skipped");
                ...
                if (total_tests_passed > 0) {
                    writer.println("<div align='left' style=\"padding-left: 30px;\">");
                    writer.println("<h3> <b>Test Cases Passed</b></h3>");
                    startResultSummaryTable("Tests Overview passed");
    ...
    private void generateMethodDetailReport(List<ISuite> suites) {
        ...
        if (total_tests_failed > 0) {
            writer.println("<div  style=\"background-color:#DCDCDC\">");
            writer.println("<div style=\"background-color:#404040\" align=\"center\">");
            writer.println("<h2 style=\"color:#FFFFFF;\"> <b>Failed Tests Details</b></h2>");
            writer.println("</div>");
            for (ISuite suite : suites) {
                Map<String, ISuiteResult> r = suite.getResults();
                for (ISuiteResult r2 : r.values()) {
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                if (mq == 0) {
                    ...
                    // titleRow(testname + " &#8212; " + style + details, 5, id); //TODO
                    m_testIndex = null;
                }
                if (!className.equalsIgnoreCase(lastClassName)) {
                    if (mq > 0) {
                        ...
                        writer.println("<tr class=\"" + style
                                ...
                                + "<td");
                        if (mq > 1) {
                            writer.println(" rowspan=\"" + mq + "\"");
                        }
                        writer.println("><b>" + lastClassName + "</b></td>" + buff);
                    }
                    mq = 0;
                    buff.setLength(0);
            ...
            if (mq > 0) {
                cq += 1;
                writer.println("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
                if (mq > 1) {
                    writer.println(" rowspan=\"" + mq + "\"");
                }
                writer.println("><b>" + lastClassName + "</b></td>" + buff);
    ...
    private void startResultSummaryTable(String style) {
        ...
        writer.println("<tr><th>Class</th>"
                + "<th>Test Case</th><th>Exception Info</th><th>Start Time </th><th>Execution Time<br/>(hh:mm:ss)</th></tr>");
        m_row = 0;
    }
    ...
    private void resultDetail(IResultMap tests) {
        ...
        for (ITestResult result : testResultsList) {
            ...
            tableStart("result", cname);
            writer.println("<tr>");
            writer.println("<td><b><h3 id=\"m" + m_methodIndex + "\">" + cname + ":" + method.getMethodName() + "</h3></b></td>");
            writer.println("<td>");
            Set<ITestResult> resultSet = tests.getResults(method);
            generateResult(result, method, resultSet.size());
            writer.println("</td></tr>");
            writer.println("</table>");
            writer.println("<p class=\"totop\"><a href=\"#summary\">back to tests-overview</a></p>");
        }
    }
    ...
    private void generateResult(ITestResult ans, ITestNGMethod method,
        ...
        if (hasParameters) {
            ...
            writer.println("<tr class=\"param\">");
            for (int x = 1; x <= parameters.length; x++) {
                writer.println("<th>Param." + x + "</th>");
            }
            writer.println("</tr>");
            writer.println("<tr class=\"param stripe\">");
        ...
        if (hasReporterOutput || hasThrowable) {
            if (hasParameters) {
                ...
                if (parameters.length > 1) {
                    writer.println(" colspan=\"" + parameters.length + "\"");
                }
                writer.println(">");
            } else {
                writer.println("<div>");
    ...
    private void generateExceptionReport(Throwable exception, ITestNGMethod method) {
        writer.println("<div class=\"stacktrace\">");
        //writer.println(Utils.stackTrace(exception, false)[0]); //TODO
        writer.println(Utils.longStackTrace(exception, false));
        writer.println("</div>");
    }
    ...
    private void summaryCellInfo(String v, String bgColor) {
        writer.println("<td class=\"numi_info\" bgcolor=\"" + bgColor + "\"\">" + v
                + "</td>");
    }
    ...
    private void writeReportTitle(String title) {
        writer.println("<div style=\"background-color:#1a1a1a\">");
    ...
    private String getDateAsString() {
        DateFormat dateFormat = new SimpleDateFormat("EEEEE MMMMM yyyy HH:mm:ss");
        Date date = new Date();
    ...
    private void startHtml(PrintWriter out) {
        ...
        out.println("<head>");
        out.println("<title>UnicornM Report</title>");
        ...
        out.println(".numi_info,.numi_attn_info {text-align:left}");
        ...
        out.println(".floatLeft { width: 30%; float: left; }");
        out.println(".floatRight {width: 60%; float: right; }");
    ...
    private void endHtml(PrintWriter out) {
        writer.println("<div style=\"background-color:#1a1a1a\">");
        writer.println("<center><h3 style=\"color:#FFFFFF;\"> www.gslab.com</h3></center>");
        writer.println("</div>");
        out.println("</body></html>");
    }

    private void generateEnvironmentReport() {
        writer.println("<div class=\"floatLeft\" style=\"padding-left: 30px;\">");
        writer.println("<h2> <b>Testing Environment</b></h2>");
        ...
        writer.println("<tr>");
        summaryCellInfo("Browser Type");
        summaryCellInfo(Config.getBrowserType());
        writer.println("</tr>");

        writer.println("<tr>");
        summaryCellInfo("Application Under Test");
        summaryCellInfo(Config.getAutUrl());
        writer.println("</tr>");

        writer.println("<tr>");
        summaryCellInfo("Host Name");
        summaryCellInfo("vivek.lande@gslab.com");
        writer.println("</tr>");
    ...
    private void generateOverallExecutionReport(List<ISuite> suites) {
        getExecutionCountSummary(suites);
        writer.println("<div class=\"floatRight\">");
        writer.println("<div style=\"padding-left:197px;\">");
    ...
    private void getExecutionCountSummary(List<ISuite> suites) {
        long time_start = 0;
        long time_end = 0;
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r : tests.values()) {
                ITestContext overview = r.getTestContext();
                total_tests_passed = total_tests_passed + getMethodSet(overview.getPassedTests(), suite).size();
                total_tests_skipped = total_tests_skipped + getMethodSet(overview.getSkippedTests(), suite).size();
                total_tests_failed = total_tests_failed + getMethodSet(overview.getFailedTests(), suite).size();

                time_start = time_start + overview.getStartDate().getTime();
                time_end = time_end + overview.getEndDate().getTime();
                total_time = timeConversion((time_end - time_start) / 1000);
            }

            SimpleDateFormat summaryFormat = new SimpleDateFormat("hh:mm:ss");
            execution_start_time = summaryFormat.format(time_start).toString();
            execution_end_time = summaryFormat.format(time_end).toString();
            execution_total_time = total_time;
        }
    }
